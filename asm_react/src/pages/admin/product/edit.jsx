...
const EditProduct = () => {
    ...
    const fetchProduct = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/products/list/${id}`
        );
        if (response.data && response.data.data) {
          ...
          setCurrentImageUrl(
            product.imageUrl || `http://localhost:3000/images/${product.images}`
          );
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        error.response?.data?.error || "Đã xảy ra lỗi khi cập nhật sản phẩm"
      );
    } finally {
      setLoading(false);
    }
